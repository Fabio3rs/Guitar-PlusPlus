...
class CLuaH {
  ...
  public:
    template <class T> class nullinit {
        T i;

      public:
        T &operator()() { return i; }

        nullinit<T> &operator=(const T &l) {
            i = l;
            return *this;
        }
        nullinit<T> &operator=(nullinit<T> &l) {
            i = l;
            return *this;
        }
    ...
    struct CloseLuaState {
        void operator()(lua_State *lua) const noexcept { lua_close(lua); }
    };
    ...
    struct luaScript {
        ...
        luaScript &
        operator=(const luaScript &script) = delete; // No copy constructor!!!

        auto clone() const -> luaScript;

        auto dumpBytecode() const -> std::vector<char>;
    ...
    std::map<std::string, std::map<std::string, luaScript>>
        files; /* std::map<pathForScripts, std::map<scriptName, scriptData>> */
    ...
    class customParam {
      ...
      public:
        ...
        std::map<std::string, customParam> &getTableData() { return tableData; }

        int getType() const noexcept { return p.type; }
        ...
        void pushToLuaStack(lua_State *L) const {
            ...
            case static_cast<int>(static_cast<unsigned int>(LUA_TNUMBER) |
                                  0xF0000000u):
        ...
        customParam(const luaVarData &data) noexcept { p = data; }
    ...
    luaScript newScriptRBuffer(const char *memf, size_t sz,
                               const std::string &name);
    luaScript newScriptRBuffer(const std::vector<char> &vec,
                               const std::string &name);
     ...
     * Run only one script
     */
    static int runScript(luaScript &lua);
    ...
    void runEventWithParamsFromContainer(int id,
     ...
     */
    void
    runInternalEventWithParams(luaScript &L, int id,
                               const multiCallBackParams_t &params) noexcept;
     ...
     */
    static luaScript &getLuaStateScript(lua_State *L);

    static std::string getGlobalVarAsString(luaScript &l,
                                            const std::string &varname);
    static const char *getGlobalVarAsString(luaScript &l,
  ...
  private:
     ...
     */
    static int runScriptWithArgs(luaScript &lua, int args);

    static void catchErrorString(lua_State *L);
    static void catchErrorString(const luaScript &L);

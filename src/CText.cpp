...
void CText::Parse() {
    ...
    for (size_t i = 0; i < fileSize; i++) {
        while (isprint((unsigned char)content[i]) == 0) {
            {
                i++;
            }
        }
        ...
        *bufferB = 0;
        ...
        int scanResult =
            sscanf(&content[i], "%127s %2047[^\t\n\r]", bufferA, bufferB);

        if (!std::string(bufferA).empty()) {
            if (*bufferA == '@') {
...
void CText::save() {
    ...
    for (auto &table : tables) {
        file << "@" << table.name << "\n";
        for (int j = 0, jsize = table.fields.size(); j < jsize; j++) {
            file << table.fields[j].name << " " << table.fields[j].content
                 << "\n";
        }
...
CText::CText(const char *name, const std::string &pushIfEmpty, bool autoParse) {
    ...
    auto pushContentsIfEmpty = [this](const std::string &pushIfEmpty,
                                      bool checkFile) {
        if (!pushIfEmpty.empty()) {

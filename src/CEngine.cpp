...
auto CEngine::getSoundBPM(unsigned int sound, double at, double interval)
    ...
    // if (!b)
    {
        result = BASS_FX_BPM_DecodeGet(sound, at, at + interval, 0,
                                       BASS_FX_BPM_BKGRND | BASS_FX_BPM_MULT2 |
                                           BASS_FX_FREESOURCE,
                                       (BPMPROGRESSPROC *)0, 0);
        // b = true;
    }
...
void CEngine::clearAccmumaltionBuffer() { glClear(GL_ACCUM_BUFFER_BIT); }
...
auto CEngine::getChannelData(int handle) -> CEngine::chdata {
    ...
    int r = BASS_ChannelGetData(handle, result.data.data(),
...
void CEngine::pushMatrix() { glPushMatrix(); }

void CEngine::popMatrix() { glPopMatrix(); }

void CEngine::multiplyMatrix(float *matrix) { glMultMatrixf(matrix); }
...
void CEngine::setCamera(const cameraSET &cam) {
    ...
    auto perspectiveM = glm::perspective(
        glm::radians(45.0), (double)windowWidth / (double)windowHeight, 0.005,
        perspectiveMaxDist);
...
static void windowCallBack(GLFWwindow *window, int w, int h) {
    ...
    auto perspectiveM = glm::perspective(
        glm::radians(45.0), (double)w / (double)h, 0.005, perspectiveMaxDist);
    ...
    if (*width == 1 || *height == 1) {
        ...
        for (m = 0; m < halfheight; m++) {
            for (n = 0; n < halfwidth; n++) {
                for (k = 0; k < components; k++) {
                    *dst++ = (GLubyte)(((int)*src + (int)src[components] +
...
auto CEngine::loadTextureImage2D(GLFWimage *img, int flags, bool glUpload)
    ...
    if ((glMajor == 1) && (glMinor == 0) && (img->Format == GL_ALPHA)) {
...
void CEngine::uploadBytesToGl(unsigned int text, const char *bytes, int width,
                              int height) {
    bindTexture(text);
    // std::cout << glGetError();
...
auto CEngine::loadTextureImage2DFinish(GLFWimage *img, int flags) const -> int {
    ...
    do {
        ...
        glTexImage2D(GL_TEXTURE_2D, level, format, img->Width, img->Height, 0,
...
void CEngine::setScale(double x, double y, double z) { glScaled(x, y, z); }
...
void CEngine::RenderMulti3DQuad(
    ...
    std::unique_ptr<GLdouble[]> textureArray(
        std::make_unique<GLdouble[]>(12 * i));

    for (int j = 0; j < i; j++) {
        /*GLdouble vA[] = { quad3DData[i].x1, quad3DData[i].y1,
           quad3DData[i].z1, quad3DData[i].x2, quad3DData[i].y2,
           quad3DData[i].z2, quad3DData[i].x3, quad3DData[i].y3,
           quad3DData[i].z3, quad3DData[i].x3, quad3DData[i].y3,
           quad3DData[i].z3, quad3DData[i].x4, quad3DData[i].y4,
           quad3DData[i].z4, quad3DData[i].x1, quad3DData[i].y1,
           quad3DData[i].z1 };*/
    ...
    /*glBufferDataARB(GL_ARRAY_BUFFER_ARB, i * 18 * sizeof(GLdouble),
    vertexArray.get(), GL_DYNAMIC_DRAW_ARB);
...
void CEngine::attribVBOBuff(int id, int size, unsigned int buffer) {
    ...
    glVertexAttribPointer(id,       // attribute
                          size,     // size
                          GL_FLOAT, // type
                          GL_FALSE, // normalized?
                          0,        // stride
...
void CEngine::disableBuf(int id) { glDisableVertexAttribArray(id); }
...
void CEngine::disablePolygonOffset() { ::glDisable(GL_POLYGON_OFFSET_FILL); }
...
void CEngine::setClearColor(const std::array<double, 4> &color) {
    glClearColor(static_cast<float>(color[0]), static_cast<float>(color[1]),
...
void CEngine::Render2DQuad(const RenderDoubleStruct &quad2DData) {
    GLdouble vertexArray[] = {quad2DData.x1, quad2DData.y1, quad2DData.x2,
                              quad2DData.y2, quad2DData.x3, quad2DData.y3,
                              quad2DData.x3, quad2DData.y3, quad2DData.x4,
                              quad2DData.y4, quad2DData.x1, quad2DData.y1};

    GLdouble textArray[] = {
        quad2DData.TextureX1, quad2DData.TextureY1, quad2DData.TextureX2,
        quad2DData.TextureY1, quad2DData.TextureX2, quad2DData.TextureY2,
        quad2DData.TextureX2, quad2DData.TextureY2, quad2DData.TextureX1,
        quad2DData.TextureY2, quad2DData.TextureX1, quad2DData.TextureY1};
...
void CEngine::RenderCustomVerticesFloat(void *vertexPtr, void *uvPtr,
    ...
    if (vbuffer == (~0)) {
        ...
        glBufferData(GL_ARRAY_BUFFER, count * sizeof(float) * 3, vertexPtr,
                     GL_STATIC_DRAW);

        glGenBuffers(1, &nvbuffer);
        glBindBuffer(GL_ARRAY_BUFFER, nvbuffer);
        glBufferData(GL_ARRAY_BUFFER, count * sizeof(float) * 3, normals,
                     GL_STATIC_DRAW);

        glGenBuffers(1, &uvbuffer);
        glBindBuffer(GL_ARRAY_BUFFER, uvbuffer);
        glBufferData(GL_ARRAY_BUFFER, count * sizeof(float) * 2, uvPtr,
                     GL_STATIC_DRAW);
    }
...
void CEngine::Render2DCircleBufferMax(double x, double y, double perone,
                                      double radius, double lineWeight,
                                      int maxPolys, unsigned int &bufferID) {
    if (perone > 1.0) {
        perone = 1.0;
    }
    ...
    bindTexture(0);
    glBufferDataARB(GL_ARRAY_BUFFER_ARB, polysNum * 12 * sizeof(GLdouble),
                    result.get(), GL_STATIC_DRAW_ARB);
...
void CEngine::Render2DCircle(double x, double y, double percent, double radius,
    ...
    bindTexture(0);
    glBufferDataARB(GL_ARRAY_BUFFER_ARB, polysNum * 12 * sizeof(GLdouble),
                    result.get(), GL_DYNAMIC_DRAW_ARB);
...
void CEngine::RenderCustomVerticesFloat(staticDrawBuffer &buffer,
    ...
    if (buffer.bufferID == (~0)) {
        glGenBuffers(1, std::addressof(buffer.bufferID));

        glBindBuffer(GL_ARRAY_BUFFER_ARB, buffer.bufferID);

        glBufferDataARB(GL_ARRAY_BUFFER_ARB, buffer.sizebytes, buffer.pointer,
                        GL_STATIC_DRAW_ARB);
    }
...
void CEngine::Render3DQuad(const RenderDoubleStruct &quad3DData) {
    GLdouble vertexArray[] = {quad3DData.x1, quad3DData.y1, quad3DData.z1,
    ...
    GLdouble textArray[] = {
        quad3DData.TextureX1, quad3DData.TextureY1, quad3DData.TextureX2,
        quad3DData.TextureY1, quad3DData.TextureX2, quad3DData.TextureY2,
        quad3DData.TextureX2, quad3DData.TextureY2, quad3DData.TextureX1,
        quad3DData.TextureY2, quad3DData.TextureX1, quad3DData.TextureY1};
...
void CEngine::pushQuad(dTriangleWithAlpha &arr,
    ...
    if (arr.useColors) {
        GLdouble alphaArray[] = {1.0, 1.0, 1.0, quad3DData.alphaTop,
...
CEngine::~CEngine() { glfwTerminate(); }

void CEngine::staticDrawBuffer::destroy(bool deletePtr) {
    if (bufferID != (~0) && glIsBuffer != nullptr &&
        glDeleteBuffers != nullptr && glIsBuffer(bufferID)) {
        // GLFWerrorfun(0, (std::string("CEngine::staticDrawBuffer::destroy
        // glDeleteBuffers ") + std::to_string(bufferID)).c_str());
...
CEngine::staticDrawBuffer::~staticDrawBuffer() { destroy(); }

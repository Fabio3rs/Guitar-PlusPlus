...
static const int max_chars_per_print =
    MAX_PATH + 256; // Max characters per Print() call
static const int symbol_max =
    256; // Max size of a symbol (func symbol, var symbol, etc)
static const int max_static_buffer = 4096; // Max static buffer for logging

// Stackdump constants
static const int stackdump_max_words =
    60; // max number of CPU words that the stackdump should dump
static const int stackdump_words_per_line = 6; // max CPU words in a single line
static const int stackdump_line_count =
    (stackdump_max_words / stackdump_words_per_line) + 1;
...
static const int max_logsize_basic =
    (MAX_PATH + 200); // module path + other text
static const int max_logsize_regs =
    32 + (4 * 4 * 28); // info + (regsPerLine * numLines * charsPerReg)
static const int max_logsize_stackdump =
    32 + 80 + (stackdump_line_count * 32) +
    (10 * stackdump_words_per_line * stackdump_line_count);
static const int max_logsize_backtrace =
    32 + max_backtrace_ever * (MAX_PATH + symbol_max + 90);
static const int max_logsize_ever = 32 + max_logsize_basic + max_logsize_regs +
                                    max_logsize_stackdump +
                                    max_logsize_backtrace;
...
static int LogException(char *buffer, size_t max,
                        LPEXCEPTION_POINTERS pException, bool bLogRegisters,
                        bool bLogStack, bool bLogBacktrace);
...
/*
 *  ExceptionTrace
 *      This class is responssible for tracing all possible informations about
 * an LPEXCEPTION_POINTER
 */
class ExceptionTracer {
  public:
  ...
  protected:
    ...
    char spc[(10 * 4) +
             1]; // Scope/spacing buffer, 4 spaces per scope, max 10 scopes
...
TheUnhandledExceptionFilter(LPEXCEPTION_POINTERS pException) {
    // Logs exception into buffer and calls the callback
    auto Log = [pException](char *buffer, size_t size, bool reg, bool stack,
                            bool trace) {
        if (LogException(buffer, size, (LPEXCEPTION_POINTERS)pException, reg,
                         stack, trace))
            ExceptionCallback(buffer);
    };

    // Try to make a very descriptive exception, for that we need to malloc a
    // huge buffer...
    if (auto buffer = (char *)malloc(max_logsize_ever)) {
        ...
        static const auto size =
            max_logsize_basic + max_logsize_regs + max_logsize_stackdump;
    ...
    return (PrevFilter ? PrevFilter(pException)
                       : EXCEPTION_CONTINUE_SEARCH); // I'm not really sure
                                                     // about this return
}

/*
 *  InstallExceptionCatcher
 *      Installs a exception handler to call the specified callback when it
 * happens with human readalbe information.
 */
...
/*
 * LogException
 *      Takes an LPEXCEPTION_POINTERS and transforms in a string that is put in
 * the logging steam
 */
static int LogException(char *buffer, size_t max,
                        LPEXCEPTION_POINTERS pException, bool bLogRegisters,
    ...
    trace.EnterScope();
    if (bLogRegisters)
        trace.PrintRegisters();
    if (bLogStack)
        trace.PrintStackdump();
    if (bLogBacktrace)
...
/*
 *  ExceptionTracer
 *      Contructs a exception trace object, responssible for tracing
 * informations about an exception
 */
ExceptionTracer::ExceptionTracer(char *buffer, size_t max,
                                 LPEXCEPTION_POINTERS pException)
    : buffer(buffer), exception(*pException),
...
/*
 *  EnterScope
 *      Enters a new scope in the logging buffer (scope is related to
 * indentation) This also prints a new line
 */
 ...
 *      Prints the well known "Unhandled exception at ..." into the logging
 * buffer
 */
void ExceptionTracer::PrintUnhandledException() {
    ...
    if (!this->module ||
        !GetModuleFileNameA(this->module, module_name, sizeof(module_name)))
        strcpy(module_name, "unknown");

    // Log the exception in a similar format similar to debuggers format
    Print("Unhandled exception at 0x%p in %s", address,
          FindModuleName(module, module_name, sizeof(module_name)));
    if (module)
        Print(" (+0x%x)", address - (uintptr_t)(module));
    Print(": 0x%X: %s", dwExceptionCode,
          GetExceptionCodeString(dwExceptionCode));

    // If exception is IN_PAGE_ERROR or ACCESS_VIOLATION, we have additional
    ...
    if (dwExceptionCode == EXCEPTION_IN_PAGE_ERROR ||
        ...
        Print(" %s 0x%p",
              rw == 0 ? "reading location"
                      : rw == 1 ? "writing location" : rw == 8 ? "DEP at" : "",
              addr);

        // IN_PAGE_ERROR have another information...
        if (dwExceptionCode == EXCEPTION_IN_PAGE_ERROR) {
            NewLine();
            Print("Underlying NTSTATUS code that resulted in the exception is "
                  "0x%p",
...
void ExceptionTracer::PrintRegisters() {
    ...
    auto PrintRegister = [this, &regs_in_line](const char *reg_name,
                                               uint32_t reg_value,
                                               const char *spaces) {
    ...
    auto PrintIntRegister = [PrintRegister](const char *reg_name,
                                            uint32_t reg_value) {
        PrintRegister(reg_name, reg_value, "  ");
    };

    // Prints a segment register
    auto PrintSegRegister = [PrintRegister](const char *reg_name,
                                            uint32_t reg_value) {
        PrintRegister(reg_name, reg_value, "   ");
    };
...
void ExceptionTracer::PrintStackdump() {
    // We need the Rsp of the exception context to execute a stack dump, make
    ...
    auto PrintWord = [this, &words_in_line](uintptr_t addr) {
        if (words_in_line++ >= stackdump_words_per_line) {
            // Print new line only if it's not the first time we enter here
            // (i.e. words_in_line has magical value)
            if (words_in_line != max_words_in_line_magic + 1)
                NewLine();
            words_in_line = 1;
            Print("0x%p: ", addr);
        }
    ...
    EnterScope();
    {
        // Makes sure the pointer at top (Rsp) is valid and readable memory
        if (VirtualQuery((void *)(top), &mbi, sizeof(mbi)) &&
            ...
            base =
                (uintptr_t)mbi.AllocationBase; // Base of the stack (uncommited)
            bottom = GetStackBottom(base);     // Bottom of the stack (commited)

            // Align the stack top (Rsp) in a 4 bytes boundary
            ...
            for (int n = 0; n < stackdump_max_words && current < bottom;
                 ++n, current += align)
                PrintWord(current);
...
void ExceptionTracer::PrintBacktrace() {
    ...
    bool has_symbol_api =
        false;         // True if we have the symbol API available for use
    ...
    EnterScope();
    {
        // Walks on the stack until there's no frame to trace or we traced
        // 'max_backtrace' frames
        while (auto trace = tracer.Walk()) {
            if (++backtrace_count >= max_backtrace)
                break;

            bool has_sym = false; // This Rip has a symbol associated with it?
            DWORD64 displacement; // Rip displacement relative to symbol

            // If we have access to the symbol api, try to get symbol name from
            // pc (Rip)
            if (has_symbol_api)
                has_sym = trace->pc ? !!SymFromAddr(GetCurrentProcess(),
                                                    (DWORD64)trace->pc,
                                                    &displacement, &symbol)
                                    : false;
            ...
            Print(backtrace_count == 1 ? "=>"
                                       : "  "); // First line should have '=>'
                                                // to specify where it crashed
            Print("0x%p ", trace->pc);          // Print Rip at frame
            if (has_sym)
                Print("%s+0x%x ", symbol.Name,
                      (DWORD)displacement); // Print frame func symbol
            Print("in %s (+0x%x) ",         // Print module
                  trace->module ? FindModuleName(trace->module, module_name,
                                                 sizeof(module_name))
                                : "unknown",
                  (uintptr_t)(trace->pc) -
                      (uintptr_t)(trace->module) // Module displacement
            );
            if (trace->frame)
                Print("(0x%p) ", trace->frame); // Print frame pointer
 ...
 */
static const char *FindModuleName(HMODULE module, char *output,
                                  size_t maxsize) {
    if (GetModuleFileNameA(module, output, maxsize)) {
        // Finds the filename part in the output string
        char *filename = strrchr(output, '\\');
        if (!filename)
            filename = strrchr(output, '/');

        // If filename found (i.e. output isn't already a filename but full
        // path), make output be filename
        if (filename) {
...
StackTracer::Trace *StackTracer::Walk() {
    if (StackWalk64(IMAGE_FILE_MACHINE_I386, GetCurrentProcess(),
                    GetCurrentThread(), &frame, &context, NULL, NULL, NULL,
                    NULL)) {
 ...
 */
void ExceptionTracer::PrintStackdump() {
    // We need the ESP of the exception context to execute a stack dump, make
    // sure we have access to it
    if ((context.ContextFlags & CONTEXT_CONTROL) == 0)
        return;

...
{
	// We need the Rsp of the exception context to execute a stack dump, make sure we have access to it
...
void ExceptionTracer::PrintBacktrace()
{
	...
	EnterScope();
	{
		...
		while (auto trace = tracer.Walk())
		{
			...
			// If we have access to the symbol api, try to get symbol name from pc (Rip)
			if (has_symbol_api)
